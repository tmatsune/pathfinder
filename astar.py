...
def algorithm(draw, grid, start, end):
    ...
    open_set_hash = {start}
    desc = {}
    while len(open_set) > 0: # algorithm will run until open set is empty, consider every possible node
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()

        current = open_set[0][2] # want node from tuple in open_set. current is current node looking at
